...
**每次 push 到 main 分支时**，自动更新三个固定的 Release：
- **V1.0.0**：按游戏分类打包（每个游戏一个 zip）
- **V1.1.0**：按角色分类打包（每个角色一个 zip）
- **V1.3.0**：按表情包分类打包（每个表情包一个 zip）
...
按表情包分类打包，每个 `Stickers` 下的一级文件夹生成一个 ZIP，并保留该文件夹作为 ZIP 内的最外层目录。例如：
...
- 仅打包 GIF、WebP、PNG、JPG、JPEG 图片（扩展名不区分大小写）。
- 图片文件名去掉扩展名后会被 App 保存为表情包备注，建议使用能表达含义或使用场景的名称。
- 用户在 App 的 **【我】→ 创意工坊 → 表情包分类** 下载并导入。
...
- 表情包目录必须至少包含一张支持格式的图片才会被打包
- 三种打包模式并行执行，提高效率
- 每次 commit 都会更新固定的 Release（V1.0.0、V1.1.0 和 V1.3.0）
...
5. **表情包目录**：每个 `Stickers` 下的一级目录对应一个独立表情包 ZIP，ZIP 内保留目录本身
6. **Release 更新**：每次 commit 都会删除并重建 V1.0.0、V1.1.0 和 V1.3.0 Release
